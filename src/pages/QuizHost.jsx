...
function QuizHost() {
  ...
  const [showJoinQr, setShowJoinQr] = useState(false) // Show join QR code
  const [showLeaderboardQr, setShowLeaderboardQr] = useState(false) // Show leaderboard QR code
  const [animatingPlayers, setAnimatingPlayers] = useState([]) // Players with animation highlight
  const [showBackConfirmation, setShowBackConfirmation] = useState(false) // Back button confirmation modal

  // Demo players system
  const [demoPlayerCount, setDemoPlayerCount] = useState(3)
  const [demoPlayerWindows, setDemoPlayerWindows] = useState([])
  ...
  useEffect(() => {
    ...
    socket.emit('create-room', {
      ...
      socket.on('player-answered', (data) => {
        console.log('Player answered:', data.playerName, data.correct ? '✓' : '✗', data.responseTime + 's', 'Bonus:', data.bonusPoints, 'NewScore:', data.newScore)
        // Update player list with answer status AND score from server
        setPlayers(prev => prev.map(p =>
          p.id === data.playerId
            ? {
                ...
                score: data.newScore !== undefined ? data.newScore : p.score  // Use server score if available
              }
            : p
        ))
      })
  ...
  const showResults = () => {
    ...
    // WICHTIG: Punkte werden vom Server berechnet und sind bereits im player.score enthalten!
    // Der Server berechnet: Grundpunkte + Geschwindigkeitsbonus (50/30/10)
    // Die Scores wurden bereits bei 'player-answered' vom Server synchronisiert
        ...
        <div className="lobby">
          <div className="lobby-content">
            <div style={{ position: 'absolute', top: '20px', right: '20px', display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'flex-end' }}>
              {/* Demo Players Controls */}
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center', background: 'rgba(255,255,255,0.1)', padding: '10px', borderRadius: '12px' }}>
                <label style={{ fontSize: '14px', fontWeight: '600' }}>Demo-Spieler:</label>
                <input
                  type="number"
                  min="1"
                  max="20"
                  value={demoPlayerCount}
                  onChange={(e) => setDemoPlayerCount(parseInt(e.target.value) || 1)}
                  style={{
                    width: '60px',
                    padding: '6px',
                    borderRadius: '6px',
                ...
                <button
                  ...
                  onClick={() => {
                    const demoNames = ['Max', 'Anna', 'Leon', 'Sophie', 'Lukas', 'Emma', 'Felix', 'Mia', 'Paul', 'Laura',
                                       'Tim', 'Sarah', 'Tom', 'Lisa', 'Jan', 'Nina', 'Ben', 'Julia', 'Finn', 'Lena']
                    const demoAvatars = ['🤖', '👤', '🎯', '⭐', '🎮', '👨', '👩', '🧑', '👧', '👦',
                                         '🦊', '🐻', '🐼', '🐨', '🐯', '🦁', '🐸', '🐧', '🦉', '🦋']

                    // Open demo player windows
                    for (let i = 0; i < demoPlayerCount; i++) {
                      setTimeout(() => {
                        const playerName = demoNames[i % demoNames.length] + (i >= demoNames.length ? (Math.floor(i / demoNames.length) + 1) : '')
                        const playerAvatar = demoAvatars[i % demoAvatars.length]

                        // Open join page in new window with auto-join
                        const width = 400
                        const height = 600
                        const left = 100 + (i * 50)
                        const top = 100 + (i * 30)
                        ...
                        const win = window.open(
                          `${window.location.origin}/join?code=${joinCode}&autoName=${encodeURIComponent(playerName)}&autoAvatar=${encodeURIComponent(playerAvatar)}&autoJoin=true`,
                          `demo-player-${i}`,
                          `width=${width},height=${height},left=${left},top=${top}`
                        )

                        if (win) {
                          setDemoPlayerWindows(prev => [...prev, win])
                        }
                      }, i * 500) // 0.5 second delay between each
                    }
                  }}
                  title="Demo-Spieler als echte Fenster öffnen"
                >
                  <Users size={18} />
                  Hinzufügen
                </button>
              ...
              <button
                ...
                onClick={() => {
                  const leaderboardUrl = `${window.location.origin}/leaderboard/${joinCode}`
                  window.open(leaderboardUrl, 'leaderboard', 'width=1200,height=800')
                }}
                title="Live Rangliste in neuem Fenster öffnen"
                style={{ width: '100%' }}
              >
                <Trophy size={18} />
                Live Rangliste
            ...
            <div className="join-info card animate-fadeIn">
              ...
              <div style={{ marginTop: '30px', textAlign: 'center' }}>
                ...
                >
                  {showJoinQr ? 'QR-Code ausblenden' : '📱 QR-Code anzeigen'}
                </button>
              ...
              {/* Live Leaderboard QR Code */}
              <div style={{ marginTop: '30px', padding: '20px', background: 'rgba(99, 102, 241, 0.1)', borderRadius: '12px', border: '2px solid #6366f1' }}>
                <h3 style={{ marginBottom: '16px', color: '#6366f1', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <Trophy size={24} />
                  Live Rangliste für Zuschauer
                </h3>
                <p style={{ marginBottom: '12px', fontSize: '14px', color: '#64748b' }}>
                  Öffne die Live Rangliste auf einem zweiten Bildschirm:
                </p>

                <button
                  className="btn btn-outline"
                  onClick={() => setShowLeaderboardQr(!showLeaderboardQr)}
                  style={{ marginBottom: '16px' }}
                >
                  {showLeaderboardQr ? 'QR-Code ausblenden' : '📱 QR-Code anzeigen'}
                </button>
                ...
                {showLeaderboardQr && (
                  <>
                    ...
                    >
                      <QRCodeSVG
                        value={`${window.location.origin}/leaderboard/${joinCode}`}
                        size={150}
                        level="H"
                        includeMargin={false}
                      />
                    ...
                    <p style={{ marginTop: '12px', fontSize: '12px', color: '#64748b' }}>
                      💡 Klicke auf den QR-Code um die Live Rangliste zu öffnen
                    </p>
                  </>
                )}
                ...
                <div style={{ marginTop: '12px' }}>
                  <button
                    ...
                    onClick={() => {
                      ...
                      alert('🔗 Leaderboard-Link kopiert!')
                    }}
                    style={{ fontSize: '14px' }}
                  >
                    <Copy size={16} />
                    Leaderboard-Link kopieren
                  </button>
                </div>
              </div>
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ textAlign: 'center', marginTop: '24px' }}>
                      <p style={{ fontSize: '18px', fontWeight: '700', color: '#6366f1', marginBottom: '8px' }}>
                        Room Code:
                      </p>
                      <p style={{ fontSize: '36px', fontWeight: '900', color: '#1e293b', letterSpacing: '4px' }}>
                        {joinCode}
                      </p>
                        ...
                        <div key={press.playerId} className="buzzer-player-card" style={{ animationDelay: `${index * 0.1}s` }}>
                          ...
                          <div style={{ display: 'flex', gap: '8px' }}>
                            ...
                            <button
                              className="btn btn-outline"
                              onClick={() => {
                                // Lock player (mark as processed without points)
                                setPlayersWhoGotPoints(prev => [...prev, press.playerId])
                                // Remove from buzzer list
                                setBuzzerPresses(prev => prev.filter(p => p.playerId !== press.playerId))
                                console.log(`❌ ${press.playerName} - Keine Punkte gegeben`)
                              }}
                              title="Keine Punkte geben und weitermachen"
                            >
                              <X size={18} />
                              Keine Punkte
                            </button>
      ...
      {showBackConfirmation && (
        ...
        >
          ...
          >
            ...
            <p style={{ margin: '0 0 24px 0', fontSize: '16px', color: '#64748b', lineHeight: '1.6' }}>
              Möchtest du das Quiz wirklich verlassen? <br/>
              <strong style={{ color: '#ef4444' }}>Das Quiz wird beendet und alle Spieler werden rausgeworfen.</strong>
            </p>
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  socket.emit('leave-room', { roomCode: joinCode })
                  socket.disconnect()
